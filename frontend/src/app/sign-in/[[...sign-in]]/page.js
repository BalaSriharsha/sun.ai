...
export default function SignInPage() {
    ...
    const signInWithOAuth = (strategy) => {
        if (!isLoaded) return;
        try {
    ...
    const submit = async (e) => {
        ...
        try {
            ...
            if (result.status === 'complete') {
                ...
                console.log(result);
                setError('Additional steps required setup.');
            }
        } catch (err) {
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '80vh', padding: '40px' }}>
            <div style={{
                ...
                <div style={{ textAlign: 'center' }}>
                    <h1 style={{ margin: '0 0 8px 0', fontSize: '24px', fontWeight: 700, color: 'var(--text-primary)' }}>Sign In to sun.ai</h1>
                    <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '14px' }}>Welcome back! Please sign in to continue.</p>
                </div>
                ...
                <div style={{ textAlign: 'center', fontSize: '13px', color: 'var(--text-secondary)', marginTop: '4px' }}>
                    ...
                    <Link href="/sign-up" style={{ color: 'var(--text-primary)', fontWeight: 600, textDecoration: 'none' }}>
                        Sign up
                    </Link>
                </div>
            </div>
